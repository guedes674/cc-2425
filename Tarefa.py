...
class Tarefa:
    ...
    def monitor_device(self, device):
        ...
        if cpu_usage > alertflow_conditions['cpu_usage']:
            self.trigger_alert(device_id, "CPU usage exceeded", cpu_usage)

        # Verificação de uso de RAM
        ram_usage = self.get_device_ram_usage(device_id)
        if ram_usage > alertflow_conditions['ram_usage']:
            self.trigger_alert(device_id, "RAM usage exceeded", ram_usage)
        ...
        for interface in device_metrics['interface_stats']:
            ...
            if packets_per_second > alertflow_conditions['interface_stats']:
                self.trigger_alert(device_id, f"Packet rate on {interface} exceeded", packets_per_second)
        ...
        if packet_loss > alertflow_conditions['packet_loss']:
            self.trigger_alert(device_id, "Packet loss exceeded", packet_loss)

        # Verificação de jitter
        jitter = self.get_link_jitter(device_id)
        if jitter > alertflow_conditions['jitter']:
            self.trigger_alert(device_id, "Jitter exceeded", jitter)
    ...
    def trigger_alert(self, device_id, alert_type, current_value):
        ...
        alert_message = f"Alert: {alert_type} on {device_id}. Current value: {current_value}"
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            sock.connect(("alertflow-server-address", 9000))  # Endereço e porta do servidor AlertFlow
            sock.sendall(alert_message.encode('utf-8'))
        print(f"Alerta enviado para o dispositivo {device_id}: {alert_message}")
